...
export async function GET(request: Request) {
  try {
    ...
    // Fetch all expense transactions for this user (negative amounts only)
    ...
    // Group monthly totals — filter by category if specified
    const monthlyTotals: Record<string, number> = {};

    for (const tx of transactions) {
      if (category !== "All" && tx.category !== category) continue;
      const monthKey = tx.date.toISOString().slice(0, 7); // "YYYY-MM"
      monthlyTotals[monthKey] = (monthlyTotals[monthKey] ?? 0) + Math.abs(tx.amount);
    }

    // Sort by month and extract amounts in chronological order
